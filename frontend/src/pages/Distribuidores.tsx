...
export default function Distribuidores() {
  ...
  const onSubmit = async (data: DistribuidorFormData) => {
    try {
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.error || 'Error al guardar el proveedor',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: error.response?.data?.error || 'Error al eliminar el proveedor',
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight">Proveedores</h1>
          <p className="text-sm text-muted-foreground">
            Gestiona los proveedores del negocio
          </p>
        </div>
        ...
        <CardContent className="p-2 sm:p-6 pt-0">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-2 py-4">
            <p className="text-xs sm:text-sm text-muted-foreground">
              {table.getRowModel().rows.length}/{distribuidores.length} proveedores
            </p>
        ...
        <DialogContent className="max-w-[95vw] sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-base sm:text-lg">
              {editingDistribuidor ? 'Editar Proveedor' : 'Nuevo Proveedor'}
            </DialogTitle>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="grid gap-3 sm:gap-4 py-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div className="space-y-2">
                  <Label htmlFor="contacto" className="text-sm">Persona de Contacto</Label>
                  <Input id="contacto" {...register('contacto')} className="text-sm" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="telefono" className="text-sm">Teléfono</Label>
                  <Input id="telefono" {...register('telefono')} className="text-sm" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input id="direccion" {...register('direccion')} className="text-sm" />
              </div>

              <div className="space-y-2">
                <Label htmlFor="notas" className="text-sm">Notas</Label>
                <Input id="notas" {...register('notas')} className="text-sm" />
              </div>
            ...
            <DialogFooter className="flex-col sm:flex-row gap-2">
              ...
              <Button type="submit" disabled={isSubmitting} className="w-full sm:w-auto">
                ...
                {editingDistribuidor ? 'Guardar Cambios' : 'Crear Proveedor'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <Dialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs sm:text-sm">
              ¿Estás seguro de que deseas eliminar el proveedor "{deletingDistribuidor?.nombre}"?
